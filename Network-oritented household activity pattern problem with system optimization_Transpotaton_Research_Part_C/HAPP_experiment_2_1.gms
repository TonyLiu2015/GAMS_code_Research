$title household activity scheduling problem
OPTIONS mip = CPLEX;
...
parameter arcs(i,j,t,s) arc travel time ;
...
agent_on_node_origin(i,t,w)                vehicle origin node flow on node i at time t with state w
agent_on_node_intermediate(i,t,w)          vehicle intermediate node flow on node i at time t with state w
agent_on_node_destination(i,t,w)           vehicle destination node flow on node i at time t with state w
mandatory_act(a)                           mandatory activity
;
